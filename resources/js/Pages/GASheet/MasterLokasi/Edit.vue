...
const form = reactive({
  // user_name: props.master_lokasi.user_name,
  name: props.master_lokasi.name,
  is_active: (props.master_lokasi.is_active == 1) ? true : false,
});
...
const submit = (
  ...
  //send data to server
  router.post(`/ga_sheet/master_lokasi/${props.master_lokasi.id}`, {
      _method: 'PUT',
      name: form.name,
...
<template>
  <LayoutAuthenticated>
    ...
    <SectionMain>
      ...
      >
        <BaseButton
          :route-name="route('master_lokasi.index')"
