...
onMounted(() => {
    ...
    form.it_tx = props.it_tx;

    props.it_tx.forEach((value, index) => {
...
const changeRuangan = async (value) => {
    try {
        const response = await axios.post(
            `/ga_sheet/it/get_alat_simbol_kondisi`,
            {
...
const submit = () => {
    //send data to server
    router.post(
        `/ga_sheet/it/${props.it_header.id}`,
        {
            _method: "PUT",
...
const formDisabled = () => {
    if (props.it_header.status == 4 || props.it_header.status == 2) {
        form.disabled = true;
    }
};
</script>
...
<template>
    <LayoutAuthenticated>
        ...
        <SectionMain>
            ...
            >
                <BaseButton
                    :route-name="route('it_head.index')"
            ...
            <CardBox class="mb-6" has-table v-if="form.it_tx.length > 0">
                <div class="overflow-x-auto">
                    ...
                    >
                        ...
                        <tbody>
                            <tr v-for="(data, id) in form.it_tx" :key="id">
                ...
                <template #footer>
                    <BaseButtons
                        ...
                        no-wrap
                    >
                        <div v-if="props.it_header.status == 1">
                            <BaseButton
                                color="success"
                                label="Setujui"
                                v-model="form.value"
                                small
                                class="w-20"
                                @click="delayApprove(props.it_header)"
                            />
                            <BaseButton
                                color="danger"
                                label="Tolak"
                                ...
                                small
                                class="w-20"
                                @click="openModalReject(props.it_header)"
                            />
                        </div>
                        <div
                            v-else-if="
                                props.it_header.status == 2 ||
                                props.it_header.status == 4
                            "
                        >
                            <BaseButton
                                color="success"
                        ...
                        <div v-else-if="props.it_header.status == 3">
                            <BaseButton
                                color="success"
                                label="Setujui"
                                v-model="form.value"
