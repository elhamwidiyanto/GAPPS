...
const get_json_index = async (page = 1, field = "") => {
    if (field) {
        filter.field_master_komponen_list = field;
        filter.direction_master_komponen_list =
            filter.direction_master_komponen_list == "asc" ? "desc" : "asc";
    }

    //HTTP request
    axios
        .get(
            `komponen/json_index?page=${page}&field=${
                filter.field_master_komponen_list
            }&direction=${filter.direction_master_komponen_list}&search=${
                filter.search_master_komponen_list
            }&search_status=${filter.search_status ? filter.search_id : ""}`,
            {}
        )
        .then((response) => {
            master_komponen.value = response.data.data;
        })
        .catch(function (error) {
            if (error.response) {
                console.log(error.response);
...
const filter = reactive({
    field_master_komponen_list: "",
    direction_master_komponen_list: "",
    search_master_komponen_list: "",
    search_status: "",
...
function destroy(id) {
    formDelete.delete(route("komponen.destroy", id));
}
    ...
    <LayoutAuthenticated>
        ...
        <SectionMain>
            ...
            >
                ...
                >
                    <BaseButton
                        :route-name="route('komponen.create')"
                ...
                <form @submit.prevent="get_json_index()">
                    <div class="-mx-3 md:flex mb-0 p-3">
                        <div class="md:w-1/2 px-3 mb-6 md:mb-0">
                            <FormControl
                                v-model="filter.search_master_komponen_list"
                                type="text"
                                placeholder="Search..."
                            >
                            </FormControl>
                        </div>
                        <div class="flex md:w-1/7 px-3 mb-2 md:mb-0 justify-end">
            ...
            <CardBox>
                <div class="overflow-x-auto">
                    ...
                    >
                        ...
                        <tbody>
                            <tr v-for="(data, index) in master_komponen.data" :key="index">
                                ...
                                <td data-label="Tanggal Dibuat" class="justify-center">
                                    {{ formatDateTime(data.created_at) }}
                                </td>
                                <td data-label="Tanggal Diperbarui" class="md-text-center ">
                                ...
                                >
                                    ...
                                    >
                                        <BaseButton
                                            :route-name="`/ga_sheet/komponen/${data.id}/edit`"
                                            color="info"
                    ...
                    <span> Total : {{ master_komponen.total }} </span>
                </div>

                <div
                    class="overflow-x-scroll w-96"
                    v-if="master_komponen.total > 5"
                >
                    <TailwindPagination
                        :data="master_komponen"
                        :number="2"
                        :limit="5"
