...
const sheet_roles = ref([]);

const get_json_index = async (page = 1, field = "") => {
    if (field) {
        filter.field_sheet_roles_list = field;
        filter.direction_sheet_roles_list =
            filter.direction_sheet_roles_list == "asc" ? "desc" : "asc";
    }

    //HTTP request
    axios
        .get(
            `sheet_roles/json_index?page=${page}&field=${
                filter.field_sheet_roles_list
            }&direction=${filter.direction_sheet_roles_list}&search=${
                filter.search_sheet_roles_list
            }&search_status=${
        ...
        .then((response) => {
            sheet_roles.value = response.data.data;
        })
        .catch(function (error) {
            if (error.response) {
                console.log(error.response);
...
const filter = reactive({
    field_sheet_roles_list: "",
    direction_sheet_roles_list: "",
    search_sheet_roles_list: "",
    search_status: "",
...
function destroy(id) {
    formDelete.delete(route("sheet_roles.destroy", id));
}
...
<template>
    <LayoutAuthenticated>
        <Head title="List Sheet Roles" />
        <SectionMain>
            <SectionTitleLineWithButton
                :icon="mdiFileDocument"
                title="List Sheet Roles"
                main
            >
                <div
                ...
                >
                    <BaseButton
                        :route-name="route('sheet_roles.create')"
                        :icon="mdiFileDocumentPlus"
                        label="Tambah Sheet Roles"
                        color="success"
                        rounded-full
                        small
                    />
                ...
                <form @submit.prevent="get_json_index()">
                    <div class="-mx-3 md:flex mb-0 p-3">
                        <div class="md:w-1/2 px-3 mb-6 md:mb-0">
                            <FormControl
                                v-model="filter.search_sheet_roles_list"
                                type="text"
                                placeholder="Search..."
                            >
                            </FormControl>
                        </div>
                        <div class="md:w-1/2 px-3 mb-0 md:mb-0">
            ...
            <CardBox>
                <div class="overflow-x-auto">
                    ...
                    >
                        ...
                        <tbody>
                            <tr
                                v-for="(data, index) in sheet_roles.data"
                                :key="index"
                            >
                                ...
                                <td data-label="User Name" class="text-center">
                                    {{ data.name }}
                                </td>
                                <td data-label="Type ID" class="text-center">
                                    {{ Typeid(data.type_id) }}
                                </td>
                                ...
                                >
                                    ...
                                    >
                                        <BaseButton
                                            :route-name="`/ga_sheet/sheet_roles/${data.id}/edit`"
                                            color="info"
                    ...
                    <span> Total : {{ sheet_roles.total }} </span>
                </div>

                <div
                    class="overflow-x-scroll w-96"
                    v-if="sheet_roles.total > 5"
                >
                    <TailwindPagination
                        :data="sheet_roles"
                        :number="2"
