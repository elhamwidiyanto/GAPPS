...
const category_preventive_breakdown = ref([]);

const get_json_index = async (page = 1, field = "") => {
    if (field) {
        filter.field_category_preventive_breakdown_list = field;
        filter.direction_category_preventive_breakdown_list =
            filter.direction_category_preventive_breakdown_list == "asc"
                ? "desc"
                : "asc";
    }
    ...
    axios
        .get(
            `category_preventive_breakdown/json_index?page=${page}&field=${
                filter.field_category_preventive_breakdown_list
            }&direction=${
                filter.direction_category_preventive_breakdown_list
            }&search=${
                filter.search_category_preventive_breakdown_list
            }&search_status=${
        ...
        .then((response) => {
            category_preventive_breakdown.value = response.data.data;
        })
        .catch(function (error) {
            if (error.response) {
                console.log(error.response);
...
const filter = reactive({
    field_category_preventive_breakdown_list: "",
    direction_category_preventive_breakdown_list: "",
    search_category_preventive_breakdown_list: "",
    search_status: "",
...
<template>
    <LayoutAuthenticated>
        <Head title="List Category Preventive Breakdown" />
        <SectionMain>
            <SectionTitleLineWithButton
                :icon="mdiFileDocument"
                title="List Category Preventive Breakdown"
                main
            >
                <div
                    class="button-group flex flex-col lg:flex-row lg:justify-end space-y-2 lg:space-y-0 lg:space-x-2 w-1/3"
                >
                    <BaseButton
                        :route-name="
                            route('category_preventive_breakdown.create')
                        "
            ...
            <CardBox>
                <div class="overflow-x-auto">
                    ...
                    >
                                        ...
                                        <div class="flex flex-col">
                                            ...
                                            <svg
                                                ...
                                                v-if="
                                                    filter.field_category_preventive_breakdown_list ==
                                                        'name' &&
                                                    filter.directiod_category_preventive_breakdown_list ==
                                                        'desc'
                                                        ? true
                                                        : false
                                                "
                        ...
                        <tbody>
                            <tr
                                v-for="(data, index) in category_preventive_breakdown.data" :key="index">
                                ...
                                >
                                    ...
                                    >
                                        <BaseButton
                                            :route-name="`/ga_sheet/category_preventive_breakdown/${data.id}/edit`"
                                            color="info"
                    ...
                    <span>
                        Total : {{ category_preventive_breakdown.total }}
                    </span>
                </div>

                <div
                    class="overflow-x-scroll w-96"
                    v-if="category_preventive_breakdown.total > 5"
                >
                    <TailwindPagination
                        :data="category_preventive_breakdown"
                        :number="2"
                        :limit="5"
