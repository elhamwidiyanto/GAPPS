...
const form = reactive({
    value: null,
    histories: props.maintenance_histories,
    historyStatus: props.maintenance_histories.map(
...
const fixedData = () => {
    for (let i = 0; i < props.maintenance_histories.length; i++) {
        const history = props.maintenance_histories[i];
        if (
    ...
    for (let i = 0; i < form.fixedArray.length; i++) {
        ...
        if (history.history_status == 0) {
            if (history.user_update_id == props.user_login) {
                form.historyText[
                    i
                ] = `Anda Mengajukan maintenance dengan alat ${history.alat_name}`;
            } else {
                form.historyText[
                    i
                ] = `${history.user_update_name} Mengajukan maintenance dengan alat ${history.alat_name}`;
            }
        } else if (history.history_status == 1) {
            if (history.user_update_id == props.user_login) {
                form.historyText[
                    i
                ] = `Anda Mengubah komponen pada alat ${history.alat_name}`;
            } else {
                form.historyText[
                    i
                ] = `${history.user_update_name} Mengubah komponen pada alat ${history.alat_name}`;
            }
        } else if (history.history_status == 2) {
            if (history.user_update_id == props.user_login) {
                form.historyText[
                    i
                ] = `Anda Menyetujui maintenance alat ${history.alat_name}`;
            } else {
                form.historyText[
                    i
                ] = `${history.user_update_name} Menyetujui maintenance alat ${history.alat_name}`;
            }
        } else if (history.history_status == 3) {
            if (history.user_update_id == props.user_login) {
                form.historyText[
                    i
                ] = `Anda Menolak maintenance alat ${history.alat_name} dengan alasan ${history.reason_reject}`;
            } else {
                form.historyText[
                    i
                ] = `${history.user_update_name} Menolak maintenance alat ${history.alat_name} dengan alasan ${history.reason_reject}`;
            }
        } else {
            if (history.user_update_id == props.user_update_name) {
                form.historyText[i] = `Anda membatalkan maintenance`;
            } else {
                form.historyText[
                    i
                ] = `${history.user_update_name} membatalkan maintenance`;
            }
        }
    }
};
...
function destroy(id) {
    formDelete.delete(route("maintenance_head.destroy", id));
}
    ...
    <LayoutAuthenticated>
        ...
        <SectionMain>
            <SectionTitleLineWithButton
                :icon="mdiHistory"
                title="History Maintenance Sheet"
                main
            >
