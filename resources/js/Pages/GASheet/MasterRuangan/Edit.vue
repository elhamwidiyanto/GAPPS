...
const form = reactive({
  // user_name: props.master_ruangan.user_name,
    name: props.master_ruangan.name,
    value_gedung: props.master_gedungs.name,
    is_active: props.master_ruangan.is_active == 1 ? true : false
});
...
const submit = (value) => {
    //send data to server
    router.post(
        `/ga_sheet/ruangan/${props.master_ruangan.id}`,
        {
            _method: "PUT",
            name: form.name,
            status: form.value,
