...
const get_json_index = async (page = 1, field = "") => {
    if (field) {
        filter.field_master_gedung_list = field;
        filter.direction_master_gedung_list =
            filter.direction_master_gedung_list == "asc" ? "desc" : "asc";
    }

    //HTTP request
    axios
        .get(
            `gedung/json_index?page=${page}&field=${
                filter.field_master_gedung_list
            }&direction=${filter.direction_master_gedung_list}&search=${
                filter.search_master_gedung_list
            }&search_status=${
        ...
        .then((response) => {
            master_gedung.value = response.data.data;
        })
        .catch(function (error) {
            if (error.response) {
                console.log(error.response);
...
const filter = reactive({
    field_master_gedung_list: "",
    direction_master_gedung_list: "",
    search_master_gedung_list: "",
    search_status: "",
...
function destroy(id) {
    formDelete.delete(route("gedung.destroy", id));
}
    ...
    <LayoutAuthenticated>
        ...
        <SectionMain>
            ...
            >
                ...
                >
                    <BaseButton
                        :route-name="route('gedung.create')"
                ...
                <form @submit.prevent="get_json_index()">
                    <div class="-mx-3 md:flex mb-0 p-3">
                        <div class="md:w-1/2 px-3 mb-6 md:mb-0">
                            <FormControl
                                v-model="filter.search_master_gedung_list"
                                type="text"
                                placeholder="Search..."
                            >
                            </FormControl>
            ...
            <CardBox>
                <div class="overflow-x-auto">
                    ...
                    >
                        ...
                        <tbody>
                            <tr v-for="(data, index) in master_gedung.data" :key="index">
                                ...
                                >
                                    ...
                                    >
                                        <BaseButton
                                            :route-name="`/ga_sheet/gedung/${data.id}/edit`"
                                            color="info"
                    ...
                    <span> Total : {{ master_gedung.total }} </span>
                </div>

                <div
                    class="overflow-x-scroll w-96"
                    v-if="master_gedung.total > 5"
                >
                    <TailwindPagination
                        :data="master_gedung"
                        :number="2"
