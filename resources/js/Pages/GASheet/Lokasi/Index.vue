...
const get_json_index = async (page = 1, field = "") => {
    if (field) {
        ...
        filter.direction_master_lokasi_list =
            filter.direction_master_lokasi_list == "asc" ? "desc" : "asc";
    }

    //HTTP request
    axios
        .get(
            `lokasi/json_index?page=${page}&field=${
                filter.field_master_lokasi_list
            }&direction=${filter.direction_master_lokasi_list}&search=${
                filter.search_master_lokasi_list
            }&search_status=${
...
function destroy(id) {
    formDelete.delete(route("lokasi.destroy", id));
}
    ...
    <LayoutAuthenticated>
        ...
        <SectionMain>
            ...
            >
                ...
                >
                    <BaseButton
                        :route-name="route('lokasi.create')"
                        :icon="mdiPlus"
                        label="Tambah Lokasi"
                        color="success"
                        rounded-full
            ...
            <CardBox>
                <div class="overflow-x-auto">
                        ...
                        <tbody>
                            <tr v-for="(data, index) in master_lokasi.data" :key="index">
                                ...
                                >
                                    ...
                                    >
                                        <BaseButton
                                            :route-name="`/ga_sheet/lokasi/${data.id}/edit`"
                                            color="info"
                    ...
                    <span> Total : {{ master_lokasi.total }} </span>
                </div>

                <div
                    class="overflow-x-scroll w-96"
                    v-if="master_lokasi.total > 5"
                >
                    <TailwindPagination
                        :data="master_lokasi"
                        :number="2"
