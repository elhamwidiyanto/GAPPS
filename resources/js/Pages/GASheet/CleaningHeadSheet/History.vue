...
const form = reactive({
    value: null,
    histories: props.cleaning_histories,
    historyStatus: props.cleaning_histories.map(
...
const fixedData = () => {
    for (let i = 0; i < props.cleaning_histories.length; i++) {
        const history = props.cleaning_histories[i];
        if (
    ...
    for (let i = 0; i < form.fixedArray.length; i++) {
        ...
        if (history.history_status == 0) {
            if (history.user_update_id == props.user_login) {
                form.historyText[
                    i
                ] = `Anda Mengajukan cleaning dengan alat ${history.alat_name}`;
            } else {
                form.historyText[
                    i
                ] = `${history.user_update_name} Mengajukan cleaning dengan alat ${history.alat_name}`;
            }
        } else if (history.history_status == 1) {
            if (history.user_update_id == props.user_login) {
                form.historyText[
                    i
            ] = `Anda Mengubah cleaning pada ruangan ${history.ruangan_name}`;
            } else {
                form.historyText[
                    i
                ] = `${history.user_update_name} Mengubah cleaning pada ruangan ${history.ruangan_name}`;
            }
        } else if (history.history_status == 2) {
            if (history.user_update_id == props.user_login) {
                form.historyText[
                    i
                ] = `Anda Menyetujui cleaning alat ${history.alat_name}`;
            } else {
                form.historyText[
                    i
                ] = `${history.user_update_name} Menyetujui cleaning alat ${history.alat_name}`;
            }
        } else if (history.status == 3) {
            if (history.user_update_id == props.user_login) {
                form.historyText[
                    i
                ] = `Anda Menolak cleaning alat ${history.alat_name} dengan alasan ${history.reason_reject}`;
            } else {
                form.historyText[
                    i
                ] = `${history.user_update_name} Menolak cleaning alat ${history.alat_name} dengan alasan ${history.reason_reject}`;
            }
        } else {
            if (history.user_update_id == props.user_update_name) {
                form.historyText[i] = `Anda membatalkan cleaning`;
            } else {
                form.historyText[
                    i
                ] = `${history.user_update_name} membatalkan cleaning`;
            }
        }
    }
};
...
function destroy(id) {
    formDelete.delete(route("cleaning_head.destroy", id));
}
...
<template>
    <LayoutAuthenticated>
        <Head title="History Cleaning Sheet" />
        <SectionMain>
            <SectionTitleLineWithButton
                :icon="mdiFileDocumentEdit"
                title="History Cleaning Sheet"
                main
            >
