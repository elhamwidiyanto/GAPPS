...
const submit = async (data) => {
    if (data == 3 && (form.reason_reject == null || form.reason_reject == "")) {
        swal({
            title: "Peringatan!",
    ...
    axios
        .post(`/ga_sheet/cleaning_head/status`, {
            id: data.id,
            status: form.value,
...
const get_json_index = async (page = 1, field = "") => {
    if (field) {
        filter.field_cleaning_list = field;
        filter.direction_cleaning_list =
            filter.direction_cleaning_list == "asc" ? "desc" : "asc";
    }
    //HTTP request
    axios
        .get(
            `cleaning/json_index?page=${page}&field=${
                filter.field_cleaning_list
            }&direction=${filter.direction_cleaning_list}&search=${
                filter.search_cleaning_list
            }&search_status=${
        ...
        .then((response) => {
            cleaning.value = response.data.data;
            getStatusLabel();
        })
        .catch(function (error) {
            if (error.response) {
...
const filter = reactive({
    field_cleaning_list: "",
    direction_cleaning_list: "",
    search_cleaning_list: "",
    search_status: "",
...
function openModalReject(value) {
    ...
    (form.id_data = value), console.log("ini modal reject");
    console.log(value);
}
                ...
                <form @submit.prevent="get_json_index()">
                    <div class="-mx-3 md:flex mb-0 p-3">
                        <div class="md:w-1/2 px-3 mb-6 md:mb-0">
                            <FormControl
                                v-model="filter.search_cleaning_list"
                                type="text"
                                placeholder="Search..."
                            >
                            </FormControl>
            ...
            <CardBox>
                <div class="overflow-x-auto">
                    ...
                    >
                        ...
                        <tbody>
                            <tr
                                v-for="(data, index) in cleaning.data"
                                :key="index"
                            >
                                ...
                                >
                                    ...
                                    >
                                        <BaseButton
                                            :route-name="`/ga_sheet/cleaning/${data.id}/history`"
                                            color="info"
                                            :icon="mdiHistory"
                                            small
                                        />
                    ...
                    <span> Total : {{ cleaning.total }} </span>
                </div>

                <div class="overflow-x-scroll w-96" v-if="cleaning.total > 5">
                    <TailwindPagination
                        :data="cleaning"
                        :number="2"
                        :limit="5"
