...
const number_of_work = ref([]);

const get_json_index = async (page = 1, field = "") => {
    if (field) {
        filter.field_number_of_work_list = field;
        filter.direction_number_of_work_list =
            filter.direction_number_of_work_list == "asc" ? "desc" : "asc";
    }

    //HTTP request
    axios
        .get(
            `number_of_work/json_index?page=${page}&field=${
                filter.field_number_of_work_list
            }&direction=${filter.direction_number_of_work_list}&search=${
                filter.search_number_of_work_list
            }&search_status=${
        ...
        .then((response) => {
            number_of_work.value = response.data.data;
        })
        .catch(function (error) {
            if (error.response) {
                console.log(error.response);
...
const filter = reactive({
    field_number_of_work_list: "",
    direction_number_of_work_list: "",
    search_number_of_work_list: "",
    search_status: "",
...
function destroy(id) {
    formDelete.delete(route("number_of_work.destroy", id));
}
...
<template>
    <LayoutAuthenticated>
        <Head title="List Number Of Work" />
        <SectionMain>
            <SectionTitleLineWithButton
                :icon="mdiFileDocument"
                title="List Number Of Work"
                main
            >
                <div
                ...
                >
                    <BaseButton
                        :route-name="route('number_of_work.create')"
                        :icon="mdiFileDocumentPlus"
                        label="Tambah Number Of Work"
                        class="text-xs"
                        color="success"
                        rounded-full
                        small
                ...
                <form @submit.prevent="get_json_index()">
                    <div class="-mx-3 md:flex mb-0 p-3">
                        <div class="md:w-1/2 px-3 mb-6 md:mb-0">
                            <FormControl
                                v-model="filter.search_number_of_work_list"
                                type="text"
                                placeholder="Search..."
                            >
                            </FormControl>
            ...
            <CardBox>
                <div class="overflow-x-auto">
                    ...
                    >
                        ...
                        <tbody>
                            <tr v-for="(data, index) in number_of_work.data" :key="index">
                                ...
                                <td data-label="Tanggal Dibuat" class="flex justify-center">
                                    {{ formatDateTime(data.created_at) }}
                                </td>
                                <td data-label="Tanggal Diperbarui" class="flex justify-center">
                                ...
                                >
                                    ...
                                    >
                                        <BaseButton
                                            :route-name="`/ga_sheet/number_of_work/${data.id}/edit`"
                                            color="info"
                    ...
                    <span> Total : {{ number_of_work.total }} </span>
                </div>

                <div
                    class="overflow-x-scroll w-96"
                    v-if="number_of_work.total > 5"
                >
                    <TailwindPagination
                        :data="number_of_work"
                        :number="2"
