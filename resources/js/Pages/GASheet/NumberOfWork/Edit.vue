...
const form = reactive({
  // user_name: props.number_of_work.user_name,
  name: props.number_of_work.name,
  is_active: (props.number_of_work.is_active == 1) ? true : false, 
});
...
const submit = (
  ...
  //send data to server
  router.post(`/ga_sheet/number_of_work/${props.number_of_work.id}`, {
      _method: 'PUT',
      name: form.name,
      ...
      onSuccess: () => {
          ...
          swal({
              title: 'Berhasil!',
              text: 'Number of work berhasil diubah!',
              icon: 'success',
              showConfirmButton: false,
              timer: 2000
          });
